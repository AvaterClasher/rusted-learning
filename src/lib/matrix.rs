...
impl Matrix {
    ...
    pub fn add(&mut self, other: &Matrix) -> Matrix {
        if self.rows != other.rows || self.cols != other.cols {
            panic!("Attempt to add of incorrect dimensions")
        }
    ...
    pub fn dot_multiply(&mut self,other: &Matrix) -> Matrix {
        if self.rows != other.rows || self.cols != other.cols {
            panic!("Attempt to dot multiply by matrix of incorrect dimensions")
        }
        let mut res = Matrix::zeros(self.rows, self.cols);
        for i in 0..self.rows {
            for j in 0..self.cols {
    ...
    pub fn subtract(&mut self,other: &Matrix) -> Matrix {
        if self.rows != other.rows || self.cols != other.cols {
            panic!("Attempt to subtract of incorrect dimensions")
        }
        let mut res = Matrix::zeros(self.rows, self.cols);
        for i in 0..self.rows {
            for j in 0..self.cols {
    ...
    pub fn map(&mut self, function: &dyn Fn(f64) -> f64) -> Matrix {
        Matrix::from(
            ...
            .map(|value| function(value)).collect()).collect())
    }
